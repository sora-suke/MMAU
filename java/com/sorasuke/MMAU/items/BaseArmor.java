...
public class BaseArmor extends ItemArmor {
	...
	public BaseArmor(ArmorMaterial material, int type, String localname) {
		...
		/*
		 * 鎧のベースクラス。 使い方 このクラスをコピペして、新しいクラスを作る コピーしたのをpreInitや、そこで呼ばれるメソッドで使う
		 * 例:メソッド名はHogeArmorとする HogeHelmet = new HogeArmor(HOGEARMOR, 0,
		 * "hoge"); ↑こんな感じ 第一引数はアーマーマテリアル、二は鎧の種類(0～3、頭～足)
		 * 三は鎧の内部名、それ+_+第二引数で指定した種類の名前が実際に設定される ここのMODだと、素材名+Armorの名前のクラスにしている
		 * ここは消しても問題ない
		 * 
		 * 0.1.0からコピペでやらなくても済むようにした
		 * 
		 */
		armorname = localname;
